...
class AudioCast {
  ...
  static void shutdown() async {
    if (currentCastState.setState == CastState.CONNECTED) {
      await disconnect();
    }
  ...
  static Future<void> connectToDevice(Device device) async {
    try {
      ...
      if (currentCastState.getState() == CastState.DISCONNECTED) {
  ...
  static Future<void> castAudioFromUrl(String url,
    ...
    try {
      ...
      switch (currentCastState.getState()) {
  ...
  static Future<void> disconnect() async {
    try {
      ...
      if (currentCastState.getState() == CastState.CONNECTING)
        throw ('Status: CastState.CONNECTING, you are currently not connected to a device.');
  ...
  static Future<void> play() async {
    try {
      if (_currentPlaybackState.getState() == PlaybackState.PLAYING)
  ...
  static Future<void> setPosition(Duration position) async {
    try {
      if (_currentPlaybackState.getState() == PlaybackState.NO_AUDIO)
  ...
  static void _refreshDeviceList() {
    var newList = <Device>{};
    adapters.forEach((adapter) => newList.addAll(adapter.devices.getState()));
